...
function Assigntask(props) {  
  ...
  const handleFinishedTask = () => {
    ...
    userRef.orderByChild("username").equalTo(executive_name).once("value", function (snapshot) {
      snapshot.forEach(function(childSnapshot) {
        if (childSnapshot.val().status == "Accepted" || childSnapshot.val().status == "Rejected") {         
            preTasks.push({
              key: childSnapshot.key,
              status: childSnapshot.val().status, 
              executive: childSnapshot.val().username,
              assistant: childSnapshot.val().recipient,
              task: childSnapshot.val().subject,
              date: childSnapshot.val().date,
            })
        }
      });
